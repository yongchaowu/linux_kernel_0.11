...
#define CODE_SPACE(addr) ((((addr)+4095)&~4095) < \			//该宏定义判断给的的线性地址是否位于当前进程的代码段当中
current->start_code + current->end_code)					// (((addr)+4095)&~4095) 用于取得该线性地址addr所在内存页面的末端地址。

static long HIGH_MEMORY = 0;	//全局变量，存放实际物理内存最高端地址

#define copy_page(from,to) \    	 // 从from处复制 1 页内存到 to 处（4k）
__asm__("cld ; rep ; movsl"::"S" (from),"D" (to),"c" (1024):"cx","di","si")

static unsigned char mem_map [ PAGING_PAGES ] = {0,};    //物理内存映射字节图
